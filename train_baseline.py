"""Train the autoregressive baseline to convergence and evaluate."""
...
from baselines.autoregressive import AutoregressiveBaseline, train_step
from evaluation.metrics import compute_all_metrics, tier_stratified_report

SEED = 42
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
MAX_VARS = 10
N_POINTS = 50  # 50 points per equation for training efficiency
...
def train_baseline(n_train=100000, n_epochs=20, batch_size=32, lr=3e-4):
    """Full training loop."""
    np.random.seed(SEED)
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, n_train_actual, batch_size):
            ...
            loss = train_step(model, obs, tgt, optimizer)
    ...
    with open('results/baseline_training.json', 'w') as f:
...
if __name__ == '__main__':
    model, train_time = train_baseline(n_train=20000, n_epochs=15, batch_size=64)
    results = evaluate_baseline(model)
